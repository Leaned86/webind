...
myApp.controller("WizardController",
  function ($rootScope, $http, $routeParams, $scope, $uibModal, $location, $translate) {
    ...
    vm.steps = [
      ...
      {
        step: 2,
        name: "Antecedentes prenatales",
        template: "templates/formationwizard/licence.html",
      },
      {
        step: 3,
        name: "Antec.Pat.Pers.(APP)",
    ...
    vm.procDgtdeliD = function () {
      ...
      return (new Date(vm.customerData.driverLicence.dateOfProcuration) >=
      new Date(vm.customerData.driverLicence.dateOfDeliverance));
    };

    vm.validProcDate = function () {
      if (vm.customerData.driverLicence.dateOfProcuration) {
        let procDate = new Date(vm.customerData.driverLicence.dateOfProcuration);
        let maxDeliDate = new Date().setDate(actDate.getDate() - 1);
        let minDeliDate = new Date(actDate.getFullYear() - 20, actDate.getMonth(), actDate.getDate());
    ...
    vm.verifyProcDate = function () {
      ...
      if (vm.customerData.driverLicence.dateOfProcuration) {
        if (vm.validProcDateRange()) {
          ...
          vm.showPDerrorLessDD = !vm.procDgtdeliD();
        } else {
          vm.showPDerror = true;
      ...
      $http.post($rootScope.urlBase + "/customer/searchByID", {
        ...
        .success(function (result) {
          if (result.status === "ok") {
            ...
            let objeData = {type: $translate.instant('ERROR')};
        ...
        content: [
          {
            columns: [
              ...
              {
                text: [
                  {text: $translate.instant('ADDRESS') + ": " + $scope.formation.place.address, style: 'subheader'},
                  {text: " ", style: 'subheader'},
        ...
        content: [
          ...
          },
          {
            columns: [
              ...
              {
                text: [
                  {text: vm.customerData.name + " " + vm.customerData.firstName, style: 'textData', alignment: "rigth"},
                ]
              }
            ]
          },
